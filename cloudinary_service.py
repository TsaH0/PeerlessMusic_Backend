...
def check_audio_exists(track_id: str) -> dict | None:
    try:
        result = cloudinary.api.resource(
            f"peerless_music/audio/{track_id}", resource_type="video"
        )
        return {
...
def upload_audio(file_path: str, track_id: str, title: str = "", artist: str = "") -> dict:
    context = {}
    if title:
        context["title"] = title
    if artist:
...
def get_all_tracks() -> list[dict]:
    ...
    try:
        ...
        for resource in result.get("resources", []):
            track_id = resource.get("public_id", "").replace("peerless_music/audio/", "")
